...
namespace Ox.Engine
{
    ...
    public static class ArrayExtension
    {
        ...
        {
            OxHelper.ArgumentNullCheck(destination, source);
            if (length > destination.Length || length > source.Length) throw new ArgumentOutOfRangeException(
                "Length of copy must be <= length of both arrays.");
        ...
        public static bool EqualsValue(this int[] first, bool[] second)
        {
            ...
            for (int i = 0; i < first.Length; ++i) if (first[i] != ToGpuBool(second[i])) return false;
            return true;
        }
