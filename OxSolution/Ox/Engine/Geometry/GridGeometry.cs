...
namespace Ox.Engine.GeometryNamespace
{
    ...
    public static class GridGeometry
    {
        ...
        /// <param name="vertexFactory">The vertex factory that creates the grid's vertices.</param>
        /// <param name="vertexFormat">The format of the grid's vertices.</param>
        ...
        public static Geometry Create(GraphicsDevice device, VertexFactory vertexFactory,
            string vertexFormat, Point gridDims, Vector2 quadScale, Vector2 textureRepetition)
        {
            OxHelper.ArgumentNullCheck(device, vertexFactory);

            Point resolution = new Point(gridDims.X + 1, gridDims.Y + 1);

            Vector3[,] vertexMap = CreateFlatVertexMap(gridDims, quadScale);
            IVertices verts = vertexFactory.CreateVertices(vertexFormat, resolution.X * resolution.Y);
        ...
        private static void PopulatePositionsAndTexCoords(IVertices verts, Vector3[,] vertexMap,
            Point gridDims, Vector2 quadScale, Vector2 textureRepetition)
        {
            Point resolution = new Point(gridDims.X + 1, gridDims.Y + 1);
